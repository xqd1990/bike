...
<html>
	...
	<script type="text/javascript">
		...
		function jump(target){
			...
			if(target >= 0 && target < total){
				...
				else location.href = "bike/fix/showScrapbikes.do?current=" + target;
			}
		}
		//翻页（快速）
		function jumpTo(){
		...
		//报废
		function scrap(bicycle_id){
			$("#bicycleid").val(bicycle_id);
			if(window.confirm("确认报废吗？")){
				$("form").attr("action","bike/fix/scrapbikes.do");
				$("form").submit();
			}	
		}
	//显示信息
  ...
  <c:if test="${message == null }">
	...
	<div class="public-nav">您当前的位置：<a javascript:void(0)>车辆报废管理</a>><a javascript:void(0)>需要报废的自行车列表</a></div>
    <div id="main" class="public-content">
		<table border="1px solid #fff" cellspacing="0px" class="public-content-cont" >
			...
			<c:forEach items="${scrapList }" var="sc" varStatus="i">
				<tr
					ondblclick="scrap(${sc.bicycle_id })">
					<td>${i.index + 1 + page.current*page.count}</td>
					<td>${sc.bicycle_code }</td>
					<td class="page"> <input class="page-btn"  type="button"
						value="报废"
						onclick="scrap(${sc.bicycle_id })" />
					</td>
				</tr>
			</c:forEach>
		</table>
  ...
  <form method="post">
		车辆编号：<input id="bicycleid"" name="bicycle_id" readonly/><br/>
