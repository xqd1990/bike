...
<html>
	...
	<script type="text/javascript">
		...
		function jump(target){
			...
			if(target >= 0 && target < total){
				...
				else {location.href = "bike/fix/showStatIn.do?current=" + target;}
			}
		}
		//翻页（快速）
		function jumpTo(){
		...
		function search(target){
			...
			if(target >= 0 && target < total){
				...
				$.post("bike/fix/showStatInSearch.do",map,function(data){
					$("#main").html(data);
				});
				}
			}
		...
		function showpile(stid,num,name){
			...
			$("form").attr("action","bike/fix/showPileIn.do");
			$("form").submit();
		}
		//显示信息
  ...
  <c:if test="${message == null }">
	...
	<div class="public-nav">您当前的位置：<a javascript:void(0)>车辆维修调入管理</a>><a javascript:void(0)>需要维修调入的自行车列表${BInfo.bicycle_code }</a>><a javascript:void(0)>需要调入的车点列表</a></div>
    ...
    <div class="page" style="text-align:left; margin-left:5%;">
  		...
  		<input type="button" class="page-btn" value="取消" onclick="location.href='bike/fix/showStatIn.do?Bicycle_Info='+${BInfo}"/></div>
  	</div>
    ...
    <form method="post">
		...
		车辆编号：<input value="${BInfo.bicycle_code }" name="bicycle_code" readonly/><br/>
	</form>
	</body>
