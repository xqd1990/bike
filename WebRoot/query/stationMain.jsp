...
<html>
	...
	<script type="text/javascript">
		...
		function jump(target){
			...
			if(target >= 0 && target < total){
				...
				else {location.href = "query/showStat.do?current=" + target;}
				}}
		//翻页（快速）
		function jumpTo(){
		...
		function search(target){
			...
			if(target >= 0 && target < total){
				...
				var map = {"station_name":name,"address":address,"current":target};
				$.post("query/showStatSearch.do",map,function(data){
					$("#main").html(data);
				});
				}}}
		//查看车点统计信息
		function showbssi(stid){
			$("#stid").val(stid);
			$("form").attr("action","query/showStatInfo.do");
			$("form").submit();}
	//显示信息
  ...
  <c:if test="${message == null }">
	...
	<div class="public-nav">您当前的位置：<a javascript:void(0)>车点信息统计</a>><a javascript:void(0)>车点列表</a></div>
    ...
    <div class="page" style="text-align:left; margin-left:5%;">
  		...
  		<input type="button" class="page-btn" value="取消" onclick="location.href='query/showStat.do'"/></div>
  	</div>
	...
	<c:forEach items="${stationList }" var="station" varStatus="i">
		<tr
			ondblclick="showbssi(${station.station_id })">
			<td>${i.index + 1 + page.current*page.count}</td>
			<td>${station.station_code }</td>
			<td>${station.station_name }</td>
			<td>${station.address }</td>
			<td>${station.bicycle_pile_num }</td>
			<td class="page"><input class="page-btn" style="width:150px;"  type="button" value="查看车点统计信息"
				onclick="showbssi(${station.station_id })" />
