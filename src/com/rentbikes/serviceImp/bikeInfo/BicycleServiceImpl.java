package com.rentbikes.serviceImp.bikeInfo;
...
public class BicycleServiceImpl implements IBicycleService {
	...
	public List<Bicycle_Station> queryStation(Bicycle_Station station,Page page) {
		...
		if(!"-1".equals(station.getStation_code()) && station.getStation_code() != null && station.getStation_code().length() > 0) map.put("station_code", station.getStation_code());
		if(!"-1".equals(station.getStation_name()) && station.getStation_name() != null && station.getStation_name().length() > 0)map.put("station_name", station.getStation_name());
		if(!"-1".equals(station.getPerson_in_charge()) && station.getPerson_in_charge() !=null && station.getPerson_in_charge().length() > 0)map.put("person_in_charge",station.getPerson_in_charge());
		if(!"-1".equals(station.getAddress()) && station.getAddress() != null && station.getAddress().length() > 0)map.put("address",station.getAddress());
